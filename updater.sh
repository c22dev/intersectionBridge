...
if curl -I --socks5-hostname localhost:8080 github.com --max-time 10 >/dev/null 2>&1; then
    ...
    if [ "$(cat .version)" != "$(curl --socks5-hostname localhost:8080 -s "https://raw.githubusercontent.com/c22dev/intersectionBridge/main/version")" ]; then
        ...
        if [ "$choiceUpd" = "Yes" ]; then
            curl --socks5-hostname localhost:8080 https://raw.githubusercontent.com/c22dev/intersectionBridge/main/intersectiond.sh > intersectiond.sh
            curl --socks5-hostname localhost:8080 https://raw.githubusercontent.com/c22dev/intersectionBridge/main/version > .version
